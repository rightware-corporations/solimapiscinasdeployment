(() => {
  ...
  card.innerHTML = `
    ...
    <form id="orcamentoForm" novalidate>
      ...
      <section class="form-step" data-step="3" hidden><div class="form-step-eyebrow">Passo 03 · Fotografias & contexto</div><h3 class="form-step-title" tabindex="-1">Mostre-nos o local</h3>
        ...
        <p class="form-note">JPG, PNG, WebP ou HEIC. As imagens serão guardadas com o pedido; se abrir o WhatsApp, não precisa de depender dele para concluir o registo.</p>
    ...
    <dialog class="success-dialog" aria-labelledby="success-title"><div class="dialog-inner"><div class="form-step-eyebrow">SOLIMA · Pedido registado</div><h3 id="success-title" class="form-step-title">Pedido recebido</h3><p>O seu pedido foi registado com sucesso.</p><p class="dialog-ticket"></p><p class="dialog-files"></p><p>A equipa SOLIMA entrará em contacto em até 24 horas.</p><div class="dialog-actions"><button type="button" class="copy-ticket">Copiar ticket</button><a class="open-whatsapp primary" target="_blank" rel="noopener">Abrir WhatsApp</a><button type="button" class="close-dialog">Fechar</button></div></div></dialog>`;
  function upload(name,title,copy,max){return `<div class="upload-card" data-upload="${name}" data-max="${max}"><h4>${title}</h4><p>${copy}</p><label class="upload-picker">Escolher imagens<input type="file" name="${name}" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple></label><div class="preview-list"></div><p class="field-error" data-upload-error></p></div>`}
  ...
  function addFiles(name,files,max,zone){
    const errors=[];for(const file of files){if(state.files[name].length>=max){errors.push(`Máximo de ${max} imagens.`);break}if(file.size>5*1024*1024){errors.push(`${file.name}: excede 5 MB.`);continue}if(!/^image\/(jpeg|png|webp|heic|heif)$/.test(file.type)){errors.push(`${file.name}: formato não suportado.`);continue}state.files[name].push(file)}
  ...
  form.addEventListener("submit",async(e)=>{e.preventDefault();if(state.busy||!validate(3))return;state.busy=true;const submit=card.querySelector(".form-submit");submit.disabled=true;status.textContent="A validar…";const data=new FormData();["customerName","phone","location","serviceType","notes","website"].forEach(n=>data.append(n,value(n)));data.append("extras",JSON.stringify([...form.querySelectorAll("[name=extras]:checked")].map(i=>i.value)));data.append("consentGiven","true");data.append("startedAt",String(startedAt));Object.entries(state.files).forEach(([name,files])=>files.forEach(file=>data.append(name,file,file.name)));
    try{status.textContent="A enviar imagens e a registar o pedido…";const res=await fetch("/api/quote-requests",{method:"POST",headers:{"Idempotency-Key":state.idempotencyKey},body:data});const body=await res.json();if(!res.ok)throw new Error(body.error||"Não foi possível concluir o pedido.");showSuccess(body)}
    catch(error){status.textContent=error.message+" Pode tentar novamente.";submit.disabled=false;state.busy=false}
  });
  function showSuccess(body){status.textContent="Pedido recebido.";dialog.querySelector(".dialog-ticket").textContent=`Ticket: ${body.ticketNumber}`;dialog.querySelector(".dialog-files").textContent=`Guardámos ${body.uploadedFiles.location} foto(s) do local e ${body.uploadedFiles.inspiration} referência(s).`;const msg=`*Pedido de Orçamento — SOLIMA*\\n\\nTicket: ${body.ticketNumber}\\nNome: ${value("customerName")}\\nContacto: ${value("phone")}\\nLocalização: ${value("location")}\\nServiço: ${labels[value("serviceType")]}\\n\\n_O pedido e as fotografias já foram registados._`;dialog.querySelector(".open-whatsapp").href=`https://wa.me/258824407120?text=${encodeURIComponent(msg)}`;state.lastFocus=document.activeElement;dialog.showModal();dialog.querySelector(".copy-ticket").focus();dialog.querySelector(".copy-ticket").onclick=async()=>{await navigator.clipboard.writeText(body.ticketNumber);dialog.querySelector(".copy-ticket").textContent="Ticket copiado"}}
